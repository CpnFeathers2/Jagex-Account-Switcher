    ...
    public class SettingsViewModel : ViewModelBase, IDisposable
    {
        ...
        public async Task BrowseRunelitePath()
        {
            try
            {
                ...
                { 
                    Title = "Select RuneLite Userdata folder",
                    AllowMultiple = false
                };
                ...
                if (folders != null && folders.Count > 0)
                {
                    ...
                    _userSettings.SaveToFile();
                    Console.WriteLine($"RunelitePath set to: {RunelitePath}");
                }
                else
                {
                    Console.WriteLine("No folder selected or file picker returned null");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error opening folder picker for RunelitePath: {ex.Message}");
        ...
        public async Task BrowseConfigurationsPath()
        {
            try
            {
                var options = new FolderPickerOpenOptions 
                { 
                    Title = "Select Configurations Folder",
                    AllowMultiple = false
                };
                ...
                if (folders != null && folders.Count > 0)
                {
                    ...
                    _userSettings.SaveToFile();
                    Console.WriteLine($"ConfigurationsPath set to: {ConfigurationsPath}");
                }
                else
                {
                    Console.WriteLine("No folder selected or file picker returned null");
            ...
            {
                Console.WriteLine($"Error opening folder picker for ConfigurationsPath: {ex.Message}");
                Console.WriteLine($"Stack trace: {ex.StackTrace}");
            }
        }
        ...
        public async Task BrowseMicrobotJarPath()
        {
            try
            {
                ...
                if (files != null && files.Count > 0)
                {
                    ...
                    _userSettings.SaveToFile();
                    Console.WriteLine($"MicroBotJarPath set to: {MicroBotJarPath}");
                }
                else
                {
                    Console.WriteLine("No file selected or file picker returned null");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error opening file picker for MicroBotJarPath: {ex.Message}");
                Console.WriteLine($"Stack trace: {ex.StackTrace}");
            }
        }
